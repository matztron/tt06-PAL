...
#  ui_in[2]   clock is generated by the FPGA
create_clock [ get_ports "uio_in\[2\]" ]  -name shft_clk -period $::env(CLOCK_PERIOD)
set_input_delay $input_delay_value -clock [ get_clocks shft_clk ] $all_inputs_wo_clk
set_output_delay $output_delay_value -clock [ get_clocks shft_clk ] [ all_outputs ]
set_clock_uncertainty $::env(SYNTH_CLOCK_UNCERTAINTY) [ get_clocks shft_clk ]
set_clock_transition $::env(SYNTH_CLOCK_TRANSITION) [ get_clocks shft_clk ]

# rp2040_clk  and  shft_clk  are mesochronous, and they never interact
set_clock_groups -asynchronous -group { rp2040_clk } -group { shft_clk }
